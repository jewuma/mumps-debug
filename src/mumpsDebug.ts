...
export default class MumpsDebugSession extends DebugSession {
  ...
  protected async launchRequest(response: DebugProtocol.LaunchResponse, args: LaunchRequestArguments) {
    ...
    try {
      ...
      // const globalConnection = new MumpsConnect();
      // await globalConnection.init(args.hostname, args.port);
      MumpsGlobalProvider.setMconnect(this._mconnect);
      MumpsJobsProvider.setMconnect(this._mconnect);
      MumpsLocksProvider.setMconnect(this._mconnect);
  ...
  protected threadsRequest(response: DebugProtocol.ThreadsResponse): void {
    response.body = {
      threads: [new Thread(MumpsDebugSession.THREAD_ID, 'thread 1')],
    };
    this.sendResponse(response);
  }
  ...
  protected stackTraceRequest(
    ...
    try {
      ...
      if (stk.count === 0 && stk.frames.length === 0) {
        // Ensure we check frames length too, as count might be total, not current
        this.sendEvent(new TerminatedEvent());
      }
      this.sendResponse(response);
    } catch (e) {
  ...
  protected scopesRequest(response: DebugProtocol.ScopesResponse): void {
    response.body = {
      scopes: [new Scope('Local', this._localScope, false), new Scope('System', this._systemScope, false)],
    };
    this.sendResponse(response);
  }
  ...
  private _checkVars(
    ...
    varBase: string,
    lastRef: string
  ): DebugProtocol.Variable | undefined {
      ...
      if (lastVar.indexCount > indexCount + 1) {
        if (lastRef !== lastVar.bases[indexCount]) {
          ...
          if (this._variableBases[lastVar.bases[indexCount]] === undefined) {
            this._variableBases[lastVar.bases[indexCount]] = this._variableHandles.create(
              lastVar.bases[indexCount] + '|' + (indexCount + 1)
            );
          }
          returnVar = {
            name,
