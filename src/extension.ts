...
export function activate(context: vscode.ExtensionContext) {
	...
	let factory: vscode.DebugAdapterDescriptorFactory = new DebugAdapterExecutableFactory();
	context.subscriptions.push(vscode.debug.registerDebugAdapterDescriptorFactory('mumps', factory));
	...
	createDebugAdapterDescriptor(session: vscode.DebugSession, executable: vscode.DebugAdapterExecutable | undefined): vscode.ProviderResult<vscode.DebugAdapterDescriptor> {
		...
		if (!this.server) {
			...
			this.server = Net.createServer(socket => {
				const session = new MumpsDebugSession();
