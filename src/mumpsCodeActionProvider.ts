...
export default class MumpsCodeActionProvider
  implements vscode.CodeActionProvider
{
  ...
  provideCodeActions(
    ...
    if (!isLintingFile(document.uri.fsPath)) {
      ...
      const codeActions: vscode.CodeAction[] = diagnostics.map((diagnostic) => {
        let fix: vscode.CodeAction = new vscode.CodeAction(
          "No Solution",
        ...
        if (
          lineTokens.length > 1 &&
          lineTokens[lineTokens.length - 1].longName === "SET" &&
          lineTokens[lineTokens.length - 2].longName === "FOR"
        ) {
          const shortNames: boolean =
            lineTokens[lineTokens.length - 1].name.length === 1;
          const isUppercase: boolean =
            lineTokens[lineTokens.length - 1].name[0] === "S";
          let indentation = 0;
    ...
    for (let i = 1; i < tokens.length; i++) {
      ...
      if (token.position === startPosition) {
        if (lineContent[startPosition - 1] === " ") {
          //First NEW argument or single argument
          if (lineContent[positionAfterToken] === ",") {
            endPosition++;
          } else {
            //Single NEW argument - delete NEW complete
            for (let j = i - 1; j >= 0; j--) {
              if (
                tokens[j].type === TokenType.keyword &&
                tokens[j].longName === "NEW"
              ) {
                startPosition = tokens[j].position;
                if (i === tokens.length) {
                  //NEW argument is last token in the line
            ...
            if (
              positionAfterToken < lineContent.length &&
              lineContent[positionAfterToken] === " "
            )
              endPosition++;
          }
        } else {
          //not the first NEW argument
          if (lineContent[startPosition - 1] === ",") {
            startPosition--;
          }
        }
  ...
  private _varNotNewed(): vscode.CodeAction {
    ...
    if (subroutineStartLine === line) {
      ...
      const range = new vscode.Range(
        new vscode.Position(line, bracketPosition + 1),
        new vscode.Position(line, endReplace),
      );
      fix.edit.replace(
        uri!,
        range,
  ...
  private _getfixedNew(
    ...
    for (; i < lineCount; i++) {
      ...
      if (
        ...
        tokens[0].longName === "NEW" &&
        tokens[0].isPostconditioned === false
      ) {
        let j = 1;
        for (; j < tokens.length; j++) {
          if (tokens[j].type !== TokenType.local) {
        ...
        const endPosition = new vscode.Position(
          i,
          tokens[j - 1].position + tokens[j - 1].name.length,
        );
        edit.replace(
          uri,
  ...
  public generateForLoop(
    ...
    vscode.window
      .showInputBox({ prompt: "Enter loop variable including indices:" })
      .then((variableName) => {
        if (variableName) {
          this._parseDb = MumpsParseDb.getInstance(document);
          ...
          if (variableParts.length === 2) {
            ...
            let forLoop =
              " " + indices[startIndex] + "=" + orderToken + variableStart;
            for (let i = 0; i <= startIndex; i++) {
              forLoop += indices[i];
              if (i < startIndex) forLoop += ",";
            }
            forLoop +=
              ")) " + quitToken + ":" + indices[0] + '=""  ' + doToken + "\n";
            if (indexCount > startIndex + 1) {
              for (let i = startIndex + 1; i < indexCount; i++) {
                forLoop +=
                  "\t" +
                  ". ".repeat(i - startIndex + indentation) +
                  setToken +
                  " " +
                  indices[i] +
                  '="" ' +
              ...
              forLoop +=
                "\t" + ". ".repeat(indexCount - startIndex + indentation);
            }
            ...
            editor
              ...
              .then(() => {
                ...
                const newEndPosition = new vscode.Position(
                  range.end.line + indexCount - startIndex,
                  99,
                );
                ...
                editor.revealRange(
                  new vscode.Range(newEndPosition, newEndPosition),
                );
              });
          }
        }
