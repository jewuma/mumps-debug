      ...
      if (errorFound) {
        ...
        if (line === -1 || line === this._linetokens.length - 1) {
          vscode.window.showErrorMessage(
            "Routine sort ended - nothing to sort found",
          );
        } else {
          ...
          if (Object.keys(this._subroutines).length !== 0) {
            ...
            for (const name of subroutineNames) {
              ...
              for (
                let line = subroutine.startLine;
                line <= subroutine.endLine;
                line++
              ) {
                this._newSortedLines.push(document.lineAt(line).text);
              }
            }
            const lastLineLength = document.lineAt(document.lineCount - 1).text
              .length;
