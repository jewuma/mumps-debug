...
export default class MumpsDiagnosticsProvider {
  ...
  private _activeSubroutine: GeneralSubroutine = {
    name: "",
    startLine: -1,
    endLine: -1,
  };
  ...
  updateDiagnostics(document: vscode.TextDocument): void {
    if (
      document &&
      document.languageId === "mumps" &&
      !isLintingFile(document.fileName)
    ) {
      ...
      if (configuration.mumps.variablesToBeIgnoredAtNewCheck !== undefined) {
        this._variablesToBeIgnored =
          configuration.mumps.variablesToBeIgnoredAtNewCheck.split(",");
      }
      ...
      if (configuration.mumps.warnIfCodeIsUnreachable !== undefined) {
        this._enableUnreachableCodeWarning =
          configuration.mumps.warnIfCodeIsUnreachable;
      }
      this._startUpdate();
    }
  }
  private async _startUpdate(): Promise<void> {
    ...
    if (this._enableVariableCheck) {
      if (this._activeSubroutine.startLine > -1) {
        this._addSymbol(
          this._activeSubroutine.name,
          this._activeSubroutine.startLine,
  ...
  public analyzeSubroutine(routine: Subroutine): void {
    ...
    if (this._enableVariableCheck) {
      ...
      for (const parameter of routine.parameters) {
        this._varStates[parameter.name] = {
          isParameter: true,
      ...
      for (let i = routine.startLine; i <= routine.endLine; i++) {
        ...
        if (Array.isArray(this._linetokens[i])) {
          for (let j = 0; j < this._linetokens[i].length; j++) {
            ...
            if (i === routine.startLine && j === 0) {
              // skip parameters
              while (
                ++j < this._linetokens[i].length &&
            ...
            if (token.type === TokenType.keyword) {
              if (token.longName === "NEW") {
                ...
                while (
                  ...
                  if (token.type === TokenType.local) {
                    ...
                    let message = "";
                    const varState: VariableState = this._varStates[varName]
                      ? this._varStates[varName]
                      : { isExcluded: token.isExcludedVariable };
                    ...
                    if (varState.isParameter) {
                      if (token.isExcludedVariable) {
                        ...
                        if (level === 0) {
                          //NEW inside higher indentation-level should be possible
                      ...
                      if (!varState.newedAtLevel) {
                        // Variable not NEWed yet - New is OK
                        ...
                        if (varState.newedAtLevel.indexOf(level) > -1) {
                          message =
                            "Variable " +
                            varName +
                            " already mentioned in NEW command";
                ...
                if (
                  ...
                  for (let k = 0; k < routine.parameters.length; k++) {
                    if (
                      !this._varStates[routine.parameters[k].name].isExcluded
                    ) {
                      this._addWarning(
                        "NEW hides formal parameters",
                        i,
                        token.position,
                        token.name.length,
                        vscode.DiagnosticSeverity.Warning,
                        "NewHidesParam:" +
                          this._varStates[routine.parameters[k].name],
                      );
                      break;
                    }
                  }
              ...
              // local variable found at a non NEW command
              const varName = token.name;
  ...
  private _checkVariableUsage(routine: Subroutine): void {
    for (const key in this._varStates) {
      ...
      if (state.isParameter && !state.isUsed && !this._isIgnoredVariable(key)) {
        this._addWarning(
          "Variable " + key + " is a formal parameter but not used",
          routine.startLine,
        ...
        state.newedAtLine &&
        state.newedAtLine.length > 0 &&
        !state.isUsed &&
        !this._isIgnoredVariable(key)
      ) {
        this._addWarning(
          "Variable " + key + " is NEWed but not used",
          state.newedAtLine[0],
  ...
  private _addWarning(
    ...
    this._diags.push({
      ...
      range: new vscode.Range(
        ...
        new vscode.Position(endline, endPosition),
      ),
      severity,
      source: "",
    });
  ...
  private _checkLine(line: number, tokens: LineToken[]): void {
    ...
    const expectedLevel =
      line === this._lineWithDo + 1 ? this._level + 1 : this._level;
    this._level = newindentationLevel;
    this._removeQuits(this._level);
    if (this._level > expectedLevel) {
      this._addWarning(
        "indentation Level wrong, found: " +
          this._level +
          ", expected: " +
          expectedLevel,
        line,
        0,
        this._getindentationPosition(line),
        vscode.DiagnosticSeverity.Warning,
        "Expectedindentation:" + expectedLevel,
      );
    }
    if (line === this._lineWithDo + 1 && this._level < expectedLevel) {
      this._addWarning(
        "Higher indentation expected after argumentless Do",
        line,
        0,
        this._getindentationPosition(line),
        vscode.DiagnosticSeverity.Warning,
    ...
    for (let tokenId = 0; tokenId < tokens.length; tokenId++) {
      // iterate over every token in actual line
      let token: LineToken = tokens[tokenId];
      if (token.type === TokenType.comment && token.name.match(/ignoreVars:/)) {
        //Check for IgnoreVars-directive
        const ignoreVarsCommentString = token.name.split("ignoreVars:")[1];
        if (ignoreVarsCommentString) {
          const ignoredVariables = ignoreVarsCommentString
            .split(",")
            .map((variableName) => variableName.trim());
          this._variablesToBeIgnored =
            this._variablesToBeIgnored.concat(ignoredVariables);
        }
      }
      if (tokenId === 0 && token.type === TokenType.label) {
        //If there was unreachable code before this label
        //save a warning
        ...
        if (tokens[1] !== undefined && tokens[1].type === TokenType.local) {
          //Begin of a parametrized subroutine
          this._routine.startLine = line;
          while (
            ++tokenId < tokens.length &&
            tokens[tokenId].type === TokenType.local
          ) {
            this._routine.parameters.push({
              name: tokens[tokenId].name,
              position: tokens[tokenId].position,
            });
          }
      ...
      if (
        ...
        if (token.type === TokenType.keyword) {
          ...
          if (
            !ifFlag &&
            (command === "QUIT" || command === "GOTO" || command === "HALT") &&
            !token.isPostconditioned
          ) {
            let hasPostcondition = false;
            if (command === "GOTO") {
              //Check if GOTO argument is postconditioned
              for (let k = tokenId + 1; k < tokens.length; k++) {
                if (
                  tokens[k].type === TokenType.entryref &&
                  !tokens[k].name.includes("&") &&
                  !tokens[k].name.includes("@")
                ) {
                  if (
                    this._labelExists(tokens[k].name) === labelExists.exists_not
                  )
                    // && !/^\+\d+$/.test(tokens[k].name))
                    this._addWarning(
                      "Entry-Reference not found",
                      line,
                      tokens[k].position,
                      tokens[k].name.length,
                      vscode.DiagnosticSeverity.Warning,
                    );
                  else if (
                    this._labelExists(tokens[k].name) === labelExists.local
                  )
                    this._addWarning(
                      "Entry-Reference can only be called locally",
                      line,
                      tokens[k].position,
                      tokens[k].name.length,
                      vscode.DiagnosticSeverity.Warning,
      ...
      if (
        token.type === TokenType.entryref ||
        token.type === TokenType.exfunction
      ) {
        if (
          this._labelExists(token.name) === labelExists.exists_not &&
          !token.name.includes("&") &&
          !token.name.includes("@")
        ) {
          this._addWarning(
          ...
          if (previousTokenName !== "$TEXT") {
            this._addWarning(
              "Entryref+Offset is not recommended",
              line,
              token.position,
              token.name.length,
              vscode.DiagnosticSeverity.Warning,
  ...
  private _checkUnreachable(line: number, token: LineToken): void {
    const isBehindQuit =
      this._isBehindQuit
        .slice(0, this._level)
        .some((state) => state === QuitState.behindQuit) ||
      this._isBehindQuit[this._level] !== QuitState.noQuit;
  ...
  private _removeQuits(level: number): void {
    for (let i = level + 1; i < 32; i++) {
      this._isBehindQuit[i] = QuitState.noQuit;
    }
  }
  ...
  private _isIgnoredVariable(variable: string): boolean {
    ...
    for (let k = 0; k < this._variablesToBeIgnored.length; k++) {
      if (
        new RegExp("^" + this._variablesToBeIgnored[k] + "$").test(variable)
      ) {
  ...
  private _addSymbol(name: string, startLine: number, endLine: number): void {
    ...
    const methodRange = new vscode.Location(
      this._uri,
