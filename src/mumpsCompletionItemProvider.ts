...
export default class CompletionItemProvider {
  ...
  private _filesToCheck: number = 0;
  ...
  provideCompletionItems(
    ...
    if (this._labelsReady && status.lineStatus === LineStatus.jumplabel) {
      const replaceRange = new vscode.Range(
        new vscode.Position(
          position.line,
          position.character - status.startString.length,
        ),
        position,
      );
      completionItems = this._findLabel(
        status.startString,
        completionItems,
        replaceRange,
      );
  ...
  private _refreshLabelDB() {
    // Look for all Labels in .m-Routines and them + Comments
  ...
  private _checkReady(dbChanged: boolean) {
    // Check if all .m Files are processed and save LabelDB then
  ...
  private _refreshLocalLabels(document: vscode.TextDocument) {
    ...
    const existingLabels = this._labelDB.labels.filter(
      (label) => label.routine === routine,
    );
    // Create a map of existing labels for quick lookup
    const existingLabelsMap = new Map(
      existingLabels.map((label) => [label.label, label]),
    );
    ...
    for (let i = 0; i < lines.length; i++) {
      if (routine) {
        if (i === 0) {
          ...
          if (
            !existingLabelsMap.has("*FL") ||
            existingLabelsMap.get("*FL")?.line !== firstLineLabel.line
          ) {
  ...
  private _refreshFileLabels(path: string) {
    // Refresh all Labels of a changed .m File
  ...
  private _findLabel(
    startstring: string,
    list: Array<vscode.CompletionItem>,
    replaceRange: vscode.Range,
  ) {
    //let hits = 0;
    let hitlist: LabelItem[] = [];
    const localLabels: LabelInformation[] = MumpsLineParser.getLabels(
    ...
    if (startstring.charAt(0) === "^") {
      ...
      if (startstring.indexOf("^") !== -1) {
        ...
        hitlist = this._labelDB.labels.filter((item) => {
          const fits =
            item.label === label && item.routine.startsWith(routinepart);
          return fits;
        });
      } else {
        for (let i = 0; i < localLabels.length; i++) {
          if (localLabels[i].name.startsWith(startstring)) {
            hitlist.push({
              routine: "",
              label: localLabels[i].name,
              line: this._document.lineAt(localLabels[i].line).text,
            });
          }
        }
        hitlist = hitlist.concat(
          this._labelDB.labels.filter((item) => {
    ...
    for (let i = 0; i < hitlist.length && i < 100; i++) {
      ...
      let label =
        item.routine !== "" ? item.label + "^" + item.routine : item.label;
...
function getLineStatus(parsed: LineObject, position: number): ItemHint {
  ...
  if (parsed.lineRoutines && parsed.lineRoutines.length > 0) {
    for (const i in parsed.lineRoutines) {
      ...
      if (
        cmd.mPostCondition !== "" &&
        position >= cmd.pcPosition &&
        position <= cmd.pcPosition + cmd.mPostCondition.length
      ) {
        startString = cmd.mPostCondition.substring(
          0,
          position - cmd.pcPosition,
        );
        if (startString.indexOf("$$") !== -1) {
        ...
        cmd.mArguments !== "" &&
        position >= cmd.argPosition &&
        position <= cmd.argPosition + cmd.mArguments.length
      ) {
