...
class MumpsLineParser {
  ...
  public checkLine(line: string): ErrorInformation {
    ...
    if (parsed.lineComment) {
      this._pushToken({
        name: parsed.lineComment.comment,
        position: parsed.lineComment.position,
      ...
      for (let i = 0; i < this._tokens.length; i++) {
        ...
        if (token.hasOwnProperty('longName')) {
          ...
          if (name.length !== longName.length) {
            outText =
              outText.substring(0, posCorrector + position) +
              longName +
              outText.substring(posCorrector + position + name.length);
  ...
  private _checkEntryRef(
    ...
    if (
      line.substring(result.position).match(entryref) &&
      line.substring(result.position).match(entryref)![0].length > 0
    ) {
      ...
      do {
        if (line.substring(result.position).match(isv)) {
          ...
          this._pushToken({
            name: sysvariable,
            position: position,
            type: TokenType.sysvariable,
            longName,
            isExcludedVariable: true,
          });
        ...
        if (inputLine[i] === ';') {
          if (!isInsideString) {
            ...
            inputObject.lineComment = {
              comment: inputLine.substring(i).substring(1),
              position: i + inputObject.expressionPosition,
            };
            return inputObject;
          }
        }
